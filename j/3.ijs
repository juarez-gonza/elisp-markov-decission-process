...
NB. this is a list of actions per state. It is the running example in explanations of this module's functions (verbs/adverbs/conjunctions).
NB. - Expected structure: (actions of first state) , (actions of second state) ,: (actions of final state - none -)
ex =: ((10 0.5 0.3 0.2) ; (25 0.2 0.7 0.1)) , ((10 0 0.75 0.25) ; (25 0 0.3 0.7)) ,: 0 $ 0

NB. fixed point iteration over bellman equations with initial value
mdp =: b_iter ^:_ v_init

NB. 'v_init' returns the initial V(state) for each state (i.e: an array filled with as many 0s as states).
v_init =: #&0 @: # @: >

NB. 'b_iter' creates a function (verb) that takes a set of values V(state) for each state
NB. returns a list with the same shape as its' input list, with the minimum costs for each state
b_iter =: dyad define
